...
{
    return "gz:" . rawurlencode(base64_encode(gzcompress($str)));
}

function uncompress($str)
{
    if(!str_starts_with($str, "gz:"))
        return $str;
    return gzuncompress(base64_decode(rawurldecode(substr($str, 3))));
}
...
function make_smaller($filename, $orientation)
{
    ...
    if($ext == "jpg"){
        imagejpeg($img_s, $fname_s, 95);
        merge_metadata($fname_s, $filename);
        imagejpeg($img_t, $fname_t, 95);
...
function gen_autogallery($filename)
{
    ...
    $chunks = explode("_", $base);

    $len = sizeof($chunks);
    if($len < 3)
        return false;

    $size_spec = $chunks[$len - 1];
    if($size_spec == "t" || $size_spec == "s"){
        $size_spec = "_$size_spec";
        $idx = $chunks[$len - 2];
        $date = $chunks[$len - 3];
        $substr = implode("_", array_slice($chunks, 0, $len - 2));
    }else{
        $size_spec = "";
        $idx = $chunks[$len - 1];
        $date = $chunks[$len - 2];
        $substr = implode("_", array_slice($chunks, 0, $len - 1));
    }

    //echo("got idx $idx and date $date\n");
    if(preg_match("/\\d+/", $idx) && preg_match("/\\d\\d\\d\\d\\d\\d\\d\\d/", $date)){
     //   echo("globbing $substr*$size_spec.{jpg,png,webp,gif}\n");
        $filenames = glob("$substr*$size_spec.{jpg,png,webp,gif}", GLOB_BRACE);
        natcasesort($filenames);
        $out = [];
        foreach($filenames as $f){
            if(preg_match("/{$substr}_\\d+{$size_spec}\\..*/", $f)){ // filter out ones that don't match size_spec
                $out[] = $f;
            }
        }
        return implode(",", $out);
...
<?php
        ...
        foreach($filenames as $filename){
            ...
            if($autogallery)
                $autogallery = $MY_URL . "upload.php?g=" . compress($autogallery);
            $id = "btn" . $idx;
            $idx++;
?>
...
{
    global $MY_URL;
    $gal_str = uncompress($_GET["g"]);
    $filenames = explode(",", $gal_str);
    foreach($filenames as $filename){
        $url = $MY_URL . rawurlencode($filename);
?>
